 ...
 * @date	2016/01/06
 */
int  CRT288x_ExeCommand(int iSendDataLen, unsigned char bySendData[] , int* iRecvDataLen, 
										unsigned char byRecvData[], unsigned char byStCode[]);
 ...
 * @see		...
 * @return	the card status:	1 no card£¬ 
 ...
 * @see		...
 * @return	card status:	1 card unlock
 *							2 card lock
 *							9 device not on-line£¬ 
 *								card lock status unknown
 * @exception ...
 *
 * @author	luowei
 * @date	2016/01/06
 ...
 * @see		...
 * @param	[in]iOnTime--indicator flash, light time: £š0x00-0xFF£©*0.25 sec
 * @param	[in]iOffTime--indicator flash, dark time: £š0x00-0xFF£©*0.25 sec
 * @return	result,0 success£¬ others error
 * @exception ...
 *
 * @author	luowei
 ...
 *					5 TYPEB CPU card£¬ 
 *					9 unknown RF type£¬ 
 *					10 T=0 contact CPU card£¬ 
 *					11 T=1 contact CPU card£¬ 
 *					20 24C01£¬ 
 *					21 24C02£¬
 *					22 24C04£¬ 
 *					23 24C08£¬ 
 ...
 *					30 SL4442£¬ SL4428, 
 *					98 no card£¬ 
 *					99 unknown card type£¬ other failure
 * @exception ...
 *
 * @author	luowei
 * @date	2016/01/06
 ...
 * @see		...
 * @return	result,		0: unknown RF card type£¬others: error£¬ 
 *						110 S50£¬ 
 *						111 S70£¬ 
 *						112 UL£¬ 
 *						120 TypeA CPU£¬ 
 ...
 * @see		...
 * @param	[in]iICType--IC card type:	10 T=0 CPU£¬ 
 *										11 T=1 CPU£¬ 
 *										20 SL4442£¬ 
 *										21 SL4428£¬ 
 *										30 AT24C01£¬ 
 ...
 *										130 TypeB CPU£¬ 
 *										210 SAM .
 * @param	[in]wChipPower--power on to operate: 
 *										0x02 cold reset activate£¬ 
 *										0x04 warm reset activate£¬ 
 *										0x08 deactive
 ...
 * @date	2016/01/06
 */
//int  CRT288x_ChipPower(int iICType, unsigned int wChipPower, unsigned char byOutChipData[], int* iOutChipDatalen);
int  CRT288x_PowerChip(int iType, unsigned int wChipPower, unsigned char byOutChipData[], int* iOutChipDatalen);
int  CRT288x_ICPower(int iType, unsigned int wChipPower, unsigned char byOutChipData[], int* iOutChipDatalen);

/** 
 * @fn		CRT288x_ChipIO() 
* @detail	communicate with IC card
 * @see		...
 * @param	[in]wChipProtocol--IC card communication protocol: 
 *																0 reader select protocol automatically£¬
 *																1 T0 protocol£¬ 
 *																2 T1 protocol.
 ...
 * @date	2016/01/06
 */
int  CRT288x_ChipIO(unsigned int wChipProtocol, int ulInChipDataLength, unsigned char lpInbChipData[], int* ulOutChipDataLength, unsigned char lpOutbChipData[]);
 ...
 * @see		...
 * @return	result, 1 the card has been activated£¬current CPU card working clock frequence is 3.57 MHZ£¬ 
 *                  2 the card has been activated£¬current CPU card working clock frequence is 7.16 MHZ£¬ 
 *                  3 not activated£¬ 
 *                  9 unknown activated status, others except 0 is error.
 * @exception ...
 *
 * @author	luowei
 * @date	2016/01/06
 ...
 * @see		...
 * @param	[in]iSamNum--switched SAM slots (1-10)	1 SAM1£¬ 
 *													2 SAM2£¬ 
 *													3 SAM3£¬ 
 *													4 SAM4
...
/** 
 * @fn		CRT288x_SAMActivation() 
 * @detail	SAM¿šÉÏµç
 * @see		...
 * @param	[in]type--	0 SAM 5V EMV
 *						1 SAM 5V ISO7816
 *						2 SAM 3V ISO7816
 * @param	[out]atr-- atr[0]: 30H  T=0 Ð­SAM
 *								31H  T=1 Ð­SAM
 * @return	0= OK
 * @exception ...
 *
 ...
 * @detail	SAM¿š·¢ËÍAPDU
 * @see		...
 * @param	[in]Type-- 0 T=0, 1 T=1
 * @param	[in]InDataLength-- ·¢ËÍÊýŸÝ³€¶È
 * @param	[in]InData-- ·¢ËÍÊýŸÝ
 * @param	[out]OutDataLength-- œÓÊÕÊýŸÝ³€¶È
 * @param	[out]OutData-- œÓÊÕÊýŸÝ
 * @return	œá¹û,0³É¹Š£¬·Ç0Ê§°Ü¡£
 * @exception ...
 *
 */
 ...
 * @see		...
 * @param	[in]iVcc--Vcc attribute:	1 5V£¬EMV activate£¬ 
 *										2 5V£¬ISO7816 activate£¬ 
 *										3 3V£¬ISO7816 activate
 ...
 * @see		...
 * @param	[in]iMode--operation mode	1 read£¬ 
 *										2 write without parity, 
 *										3 write with parity
 * @param	[in]i24CxxType--24Cxx card type.	20 24C01card£¬ 
 *												21 24C02card£¬
 *												22 24C04card£¬ 
 *												23 24C08card£¬ 
 *												24 24C16card£¬ 
 *												25 24C32card£¬ 
 *												26 24C64card
 * @param	[in]wStartAddr--beginning position (	24C01: 0x00-0x07F, 
 *													24C02: 0x00-0x00FF, 
 *													24C04: 0x00-0x01FF, 
 *													24C08: 0x00-0x03FF,
 *													24C16: 0x00-0x07FF,
 ...
 * @see		...
 * @param	[in]iMode--operation mode   1 check password
 *										2 download password to EERPROM£
 *										3 modify password
 * @param	[in]iKs--password type		0 KeyA
 *										1 KeyB
 * @param	[in]iSn--sector number  S50 card sn=00H-0FH 
 *									S70 card sn=00H-27H
 ...
 * @see		...
 * @param	[in]iMode--operation mode   1 read sector block data
 *										2 write sector block data 
 *										3 S50 S70 initialization
 *										4 S50 S70 read balance 
 *										5 S50 S70 increment
 *										6 S50 S70 decrement
 * @param	[in]iSn--sector number (	Ultralight Card:	iSn=00H-0FH, 
 *										S50 card:			iSn=00H-0FH, 
 *										S70 card:			iSn=00H-20H (iSn=21H-27H  S70 card has 16 blocks in every sector among final 8 sectors))
 * @param	[in]iBn--operate start block	Ultralight Card:	iBn=00H, 
 *											S50 card:			iBn=00H-03H, 
 *											S70 card:			iBn=00H-03H  (iBn=00H-0FH S70 card has 16 blocks in every sector among final 8 sectors))
 * @param	[in]iLc--operation block number	Ultralight Card:	iLc=01H-10H, 
 *											S50 card:			iLc=01H-04H, 
 *											S70 card:			iLc=01H-04H  (iLc=01H-10H   S70 card has 16 blocks in every sector among final 8 sectors))
