...
void tripleDEA::IP(LWORD state[], const BYTE in[])
{
...
void tripleDEA::InvIP(LWORD state[], BYTE in[])
{
...
LWORD tripleDEA::f(LWORD state, const BYTE key[])
{
	BYTE lrgstate[6]; //,i;
	LWORD t1,t2;
...
void tripleDEA::des_key_setup(const BYTE key[], BYTE schedule[][6], DES_MODE mode)
{
	LWORD i, j, to_gen, C, D;
	const LWORD key_rnd_shift[16]= {1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1};
	const LWORD key_perm_c[28]	= {56,48,40,32,24,16,8,0,57,49,41,33,25,17,
									9,1,58,50,42,34,26,18,10,2,59,51,43,35};

	const LWORD key_perm_d[28]	= {	62,54,46,38,30,22,14,6,61,53,45,37,29,21,
									13, 5,60,52,44,36,28,20,12,4,27,19,11, 3};
	const LWORD key_compression[48] = {	13,16,10,23, 0, 4, 2,27,14, 5,20, 9,
...
void tripleDEA::des_crypt(const BYTE in[], BYTE out[], const BYTE key[][6])
{
	LWORD state[2],idx,t;
...
void tripleDEA::three_des_key_setup(const BYTE key[], BYTE schedule[][16][6], DES_MODE mode)
{
	if (mode == DES_ENCRYPT) {
...
//
void tripleDEA::three_des_crypt(const BYTE in[], BYTE out[], const BYTE key[][16][6])
{
...
//
int tripleDEA::Three_DES_Encrypt(unsigned char * Challenge, unsigned char * out, int pKey)
{
	const unsigned char Key_PSAM[24]	=	{	0xFC,0xD0,0x69,0x21,0xA3,0x75,0x2B,0x82,
												0x61,0x07,0xA3,0xF6,0xC5,0xB4,0xD2,0x7C,
												0xFC,0xD0,0x69,0x21,0xA3,0x75,0x2B,0x82
											};
	const unsigned char Key_ISAM[24]	=	{	0x7A,0x52,0x9D,0x81,0x37,0xF1,0x9B,0xB6,
												0x8A,0x81,0x38,0x53,0x68,0x91,0x8A,0x1C,
												0x7A,0x52,0x9D,0x81,0x37,0xF1,0x9B,0xB6
											};
	unsigned char schedule[16][6];
	unsigned char three_schedule[3][16][6];
	
	DES_MODE ENCRYPT,DECRYPT;
	ENCRYPT = DES_ENCRYPT;
	DECRYPT = DES_DECRYPT;
    
	if(pKey == 1)		
		three_des_key_setup(Key_PSAM, three_schedule, ENCRYPT);

	if(pKey == 2)
		three_des_key_setup(Key_ISAM, three_schedule, ENCRYPT);

	three_des_crypt(Challenge,out,three_schedule);
